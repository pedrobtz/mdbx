...
`mdbx_keys()` and `mdbx_items()` walk the database in key order, in a single crossing into C
...
Keys come back in key order, not insertion order. Both accept `as = "raw"`, needed for keys or
values that are not text, and both take `limit` to bound the read. A scan with no `limit` refuses
to run past `mdbx_scan_max` (a million records) rather than quietly materializing the lot — pass
`limit = n`, or `limit = Inf` to mean it.
